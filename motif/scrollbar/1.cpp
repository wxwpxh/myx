...
while(!feof(fp)){
fread(value,sizeof(char),1,(FILE*)fp); 
...
void get_sb(Widget pb,XtPointer client_data,XtPointer call_data)
{
 Widget scrollbar;
...
void sb_inc(Widget w,XtPointer client_data,XtPointer call_data)
{
 /*
 Widget text;
 XmTextPosition pos;
...
int main( int argc,char **argv)
 ...
 text_w=XmCreateScrolledText(rowcol,(char *)"text_w",args,i);  
 ...
 XtAddCallback(sb,XmNincrementCallback,sb_inc,text_w); 
  ...
  ReadFiletoXmtext((char *)"./makefile",text_w);
