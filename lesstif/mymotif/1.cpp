...
void ok_callbk(Widget w,XtPointer client_data,XtPointer call_data)
{
 ...
 if(!XmStringGetLtoR(cbs->value,(XmStringCharSet)XmSTRING_DEFAULT_CHARSET,&filename))
     return;
 if(!filename){
...
void exec_cmd(Widget text,XmString cmdstring)
{
 ...
 XmTextPosition pos;
 FILE *fp;
 XmStringGetLtoR(cmdstring,(XmStringCharSet)XmSTRING_DEFAULT_CHARSET,&cmd); 
 if(!cmd||!*cmd){
   if(cmd) 
    XtFree(cmd);
   return; 
...
int main( int argc,char **argv)
 ...
 Widget command,cascadeb2,c2b1,c2b2,c2b3sub,sub21_b1,sub2_panel1;
 ...
 XmString bnt_text=XmStringCreateLtoR((char*)"EXIT",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
 ...
 bnt_text=XmStringCreateLtoR((char*)"COPY",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
 XtSetArg(args[i],XmNlabelString,bnt_text);i++; 
 ...
 bnt_text=XmStringCreateLtoR((char*)"select",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
 XtSetArg(args[i],XmNlabelString,bnt_text);i++; 
 c2b2=XtCreateManagedWidget("button",xmPushButtonWidgetClass,mypanel2,args,i); 
 XmStringFree(bnt_text);
 scrolled_list_form=create_scrolled_list(c2b2); 
 ...
 i=0;    
 sub2_panel1=XmCreatePulldownMenu(mypanel2,(char*)"sub2_panel1",args,i);


 bnt_text=XmStringCreateLtoR((char*)"cascade",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;  
 XtSetArg(args[i],XmNsubMenuId,sub2_panel1);i++; 
 XtSetArg(args[i],XmNlabelString,bnt_text);i++;  
 c2b3sub=XtCreateManagedWidget("button",xmCascadeButtonWidgetClass,mypanel2,args,i);
 


 bnt_text=XmStringCreateLtoR((char*)"SUBMENU1",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
 XtSetArg(args[i],XmNlabelString,bnt_text);i++; 

 sub21_b1=XtCreateManagedWidget("button",xmPushButtonWidgetClass,sub2_panel1,args,i);
 XmStringFree(bnt_text);  
...
//create a scrolledtext for myform
 ...
 bnt_text=XmStringCreateLtoR((char*)"OPEN",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
 XtSetArg(args[i],XmNlabelString,bnt_text);i++; 
 ...
 XmString file_title=XmStringCreateLtoR((char*)"open file to view",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0; 
 ...
 bnt_text=XmStringCreateLtoR((char*)"command",(char*)XmSTRING_DEFAULT_CHARSET);
 i=0;
